...
app.get("/specific-event/:id", (req, res)=>{
    ...
    Companie.find({"events._id":id}).then((response)=>{
        const newResponse = []
        response.forEach((element)=>{
            element.events.forEach((list)=>{
                if(list._id == req.params.id){newResponse.push(list)}
            })
        })
        res.send(newResponse)
    }).catch((err)=>{
...
app.post("/register-empresa", async (req,res)=>{
    const {logo, name, empresa, user ,email, password, cellphone, cpf ,url, } = req.body
    try{
        console.log(logo)
...
app.post("/login-cliente", (req,res)=>{
    ...
    Costumer.find({email:email, password:password}).then((response)=>{
        res.send(response)
        console.log(response)
    }).catch((err)=>{
        console.log(err)
...
app.post("/login-empresa", (req,res)=>{
    let {email, password} = req.body  
    Companie.find({email:email, password:password}).then((response)=>{
        res.send(response)
        console.log(response)
    }).catch((err)=>{
        console.log(err)
...
app.get("/empresa/:id", async (req, res)=>{
    const id = req.params.id
    Companie.findOne({_id:req.params.id}).then((response)=>{
        res.send(response)
    }).catch((err)=>{
        console.log(err)
    })
...
app.post("/set-ingresso", async (req,res)=>{
    try{
        const {
            empresa, 
            image, 
